    ...
    >
      ...
      <section className={``} >
        <div className="pt-[5px] xl:pt-[75px] 2xl:pt-[135px]   !font-colasta container">
          <div className="flex gap-5 md:gap-[40px] flex-col md:flex-row  2xl:pb-[135px]">
            <h1
            ...
            >
              <p style={{fontFamily: "Instrument, sans-serif"}}
                className="tracking-wider text-primary font-medium">
              Best blogs, tips, tricks, articles for all your AI needs.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="relative mx-auto max-w-7xl">
            ...
            <div className="mx-auto mt-8 md:mt-12 grid max-w-lg gap-x-6 gap-y-16 lg:max-w-none lg:grid-cols-3">
              {data.ebooks.map((ebook) => (
                <div key={ebook.title} className="flex flex-col overflow-hidden   group cursor-pointer">
                  <div className="flex-shrink-0 w-full bg-gray-100 flex items-center justify-center rounded-2xl h-64 mb-6">
                    <Image width={500} height={500} className="h-52 w-full object-contain  group-hover:opacity-50" src={ebook.cover_image} alt="" />
                  </div>
                  <div className="flex flex-1 flex-col justify-between  px-2">
                    <div className="flex-1">
                      <p className="text-sm font-medium text-indigo-800">
                        <Link href={`/ebook/${ebook.slug}`} className="hover:underline">
                          Ebook
                        </Link>
                      </p>
                      <Link href={`/ebook/${ebook.slug}`} className="mt-2 block">
                        <p className="text-xl font-semibold text-gray-900">{ebook.title}</p>
                      </Link>
                    </div>
                    <div className="mt-6 flex items-center">
                      <div className="flex-shrink-0">
                        {/* <Link href={`/blog/${ebook.slug}`}>
                        <span className="sr-only">{post.author}</span>
                      </Link> */}
                      </div>
                      <div className="">
                        <p className="text-sm  text-gray-600 group-hover:text-indigo-800">
                          {/* <Link href={`/blog/${ebook.slug}`} className=" flex items-center gap-3"> */}
                          {/* <span className="block uppercase w-12 h-12 flex  items-center justify-center text-2xl font-medium rounded-full bg-gray-200 shrink-0">{ebook.author[0]}</span> By {post.author} •  {post.time_to_read} min read  */}
                          {/* </Link> */}
                          <Link
