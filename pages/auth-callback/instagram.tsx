...
function AuthCallbackForInstagram() {
  ...
  async function handleInstagramAuth() {
    ...
    if (!accessToken) {
      setMessage("Access token was not returned by Instagram.");
      startClosingTimer();
      return;
    }
    if (!state) {
    ...
    try {
      await axiosWithoutAuth.post("/communication-channels/setup-instagram", {
      ...
      <div className="p-8 bg-white rounded-lg shadow-md text-center">
        ...
        {timeLeft < 5 && (
          <p className="text-sm text-gray-500">
            Closing in {timeLeft >= 1 ? timeLeft : `0`} second
            {timeLeft !== 1 ? "s" : ""}...
          </p>
        )}
      </div>
