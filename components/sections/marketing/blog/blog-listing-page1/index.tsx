          ...
          <div className="relative mx-auto max-w-7xl">
            ...
            <div className="mx-auto mt-8 md:mt-12 grid max-w-lg gap-x-10 gap-y-16 lg:max-w-none lg:grid-cols-3">
              {data.map((post) => (
                <div
                  key={post.title}
                  className="flex flex-col overflow-hidden   rounded-3xl bg-white group cursor-pointer"
                >
                  <Link href={`/blog/${post.slug}`} className="flex-shrink-0  mb-6">
                    <Image
                      width={500}
                      height={500}
                    ...
                    <div className="flex-1">
                      <div className="text-sm flex items-center justify-between font-medium text-indigo-800">
                        ...
                        <p className="text-sm text-gray-500">
                          {new Date(post.published_at).toDateString()}</p>
                      </div>
