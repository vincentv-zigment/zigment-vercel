...
import ChatBotMessageSection from "./ChatBotMessageSection";
...
const ChatBotRightAway = ({
  ...
  useEffect(() => {
    if (!skip_cookie && !companyData.loading) {
      startOrVerifySession(true);
    }
  }, [companyData]);

  // Function to start a chat session or verify an existing one
  const startOrVerifySession = async (poll_after_loading: boolean = false) => {
    try {
      ...
      let sessionData = savedSessionData ? JSON.parse(savedSessionData) : {};
