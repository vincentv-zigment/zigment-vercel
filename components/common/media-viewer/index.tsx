...
function MediaViewer({ url, mime_type, is_safe, caption }: MediaFileTypes) {
    ...
    if (mime_type) {
        if (mime_type.startsWith('image/')) {
            ...
            if (nsfw && !showAnyway) {
                return (
                    <div className="relative">
                        <img src={url} alt={`${caption && caption.length > 0 ? `${caption}` : `Media content`}`} className="filter blur-xl" />
            ...
            return <img src={url} alt={`${caption && caption.length > 0 ? `${caption}` : `Media content`}`} onDoubleClick={() => {
