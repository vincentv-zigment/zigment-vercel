...
type Props = {
  ...
  list: { id: any, label: string, value: string, imgURL: string, enabled?:boolean }[];
  name: any;
  inputRef?: any;
  required?: boolean;
  position?: "up" | "down";
...
export default function CustomMultiSelectImages({
  ...
  const [selected, setSelected] = useState<any>(list.find((x) => x.value === state) ?? list[0]);
  ...
  useEffect(() => {
    setSelected(list.find((x) => x.value === state));
  }, [state]);
