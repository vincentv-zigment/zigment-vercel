...
const Footer: React.FC = () => {
  ...
  const handleSubmit = async () => {
    setLoading(true);
    try {
      await fetch(`${process.env.REACT_APP_API_URL}/cms/new-lead`, {
        method: "POST",
        headers: {
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-10">
          ...
          <div className="md:mx-auto w-full">
            ...
            <input
              ...
              className="w-full md:max-w-sm rounded-xl bg-brand-gray-400 border-none outline-brand-orange-main text-sm md:text-base py-3 px-4 mt-3 text-black focus:ring-0"
              value={email}
              onChange={(e) => setEmail(e.target.value)}
            />
            <button
              id="email"
              className="w-full md:w-40 text-center text-sm md:text-base text-brand-orange-main border border-brand-orange-mainbg font-semibold bg-brand-orange-mainbg rounded-xl hover:text-white hover:bg-brand-orange-main py-3 px-9 mt-3 md:mt-6 transitiona-all"
